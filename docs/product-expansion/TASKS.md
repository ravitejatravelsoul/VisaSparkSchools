...
| P7.8  | Go Programming course                  | completed   | `content/lessons/go.ts` (12 lessons, 4 modules) + `content/projects.ts`'s `go-cli-task-tracker` capstone; every lesson uses a `guidedOutputLab` (predict/fill-in-blank/guided-editing) since Go has no safe in-browser execution; `courseId` linked back from the `go` technology-directory entry; content:validate + real Chromium e2e both pass |
| P7.9  | Kotlin Fundamentals course             | pending     |                                                                                                                                                                                                                                                                                                                                                   |
| P7.10 | Tests/content validation               | in progress | Go's own tests pass (see P7.8); full suite validation across all 8 languages pending the remaining 7 courses                                                                                                                                                                                                                                      |
